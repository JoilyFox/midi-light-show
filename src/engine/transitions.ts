...
export class TransitionEngine {
  ...
  constructor(private driver: WizUdpDriver) {}

  getBrightness(ip: string): number { return this.current.get(ip) ?? 0; }

  /** Fade current → toPct over durationMs. Optional color at the start; optional onComplete when it lands. */
  start(ip: string, toPct: number, durationMs: number, opts: { color?: Rgb; curve?: Curve; onComplete?: () => void } = {}): void {
    this.cancel(ip);
    ...
    const timer = setInterval(() => {
      ...
      if (t >= 1) { this.cancel(ip); this.apply(ip, to); opts.onComplete?.(); }
    }, MIN_STEP_MS);
    this.timers.set(ip, timer);
  }

  /** One shot: fast attack up to toPct, then slow release down to 0. A beat "flash with decay". */
  pulse(ip: string, toPct: number, attackMs: number, releaseMs: number, curve: Curve = 'easeOut'): void {
  ...
  cancel(ip: string): void {
    ...
    if (t) { clearInterval(t); this.timers.delete(ip); }
  }
  cancelAll(): void {
